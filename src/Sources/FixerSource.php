...
class FixerSource extends Source implements SourceContract
{
    /**
    ...
    public function fetch()
    {
        ...
        if (isset($response['error']['code'])) {
            switch ($response['error']['code']) {
                case 101: // No API Key was specified or an invalid API Key was specified.
                ...
                case 105: // The current subscription plan does not support this API endpoint.
                    if ($this->defaultCurrency === "EUR") {
                        throw LimitationException::subscriptionPlanLimited();
                    }

                    throw LimitationException::subscriptionPlanLimited(
                        "Free subscription plan of [{$this->name()}] source supports only EUR as base currency."
                    );
                case 201: // An invalid base currency has been entered.
                    throw InvalidArgumentException::currencyNotSupported($this->name(), $this->defaultCurrency);
                case 202: // One or more invalid symbols have been specified.
