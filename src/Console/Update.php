...
class Update extends Command
{
	 ...
	 * @var string
	 */
	protected $signature = 'currency:update {currency? : Optional currency to be updated, otherwise all will be updated}
	...
	private function updateFromFixer($defaultCurrency)
	{
		...
		$this->updateRates($result->rates, $timestamp);
	...
	private function updateFromOpenExchangeRates($defaultCurrency, $api)
	{
		...
		$this->updateRates($content->rates, $timestamp);
	...
	/**
	 * Update the rates with the given timestamp
	 *
	 * @param $rates
	 * @param $timestamp
	 */
	private function updateRates($rates, $timestamp)
	{
		//If we're updating only one rate
		if ($currency = $this->argument('currency')) {
			/** @var \stdClass $rates */
			if (isset($rates->{$currency})) {
				...
				$this->error('The currency cannot be found.');
			}
